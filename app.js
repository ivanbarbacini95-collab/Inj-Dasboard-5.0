...
const ROLLING_IDS=new Set([
  ...
  'priceChange','dayLow','dayHigh','availableInj','availableUsd','stakedInj','stakedUsd',
  'rewardsInj','rewardsUsd',
...
function historyChange(ms){
  const rows=state.netWorthHistory||[]; if(rows.length<2) return null;
  const latest=rows.at(-1); const target=latest.t-ms;
  let base=rows[0]; for(const row of rows){ if(row.t<=target) base=row; else break; }
  if(!base?.v) return null; return {usd:latest.v-base.v,pct:((latest.v/base.v)-1)*100};
}
...
function renderHeatmap(){
  const host=$('rewardHeatmap'); if(!host) return; const daily=state.staked*(state.apr/100)/365; const stored=storage.getJSON(`inj_reward_heat_v4_${state.address||'guest'}`,[]); const today=new Date().toISOString().slice(0,10);
  let rows=Array.isArray(stored)?stored:[]; const idx=rows.findIndex(x=>x.d===today); const point={d:today,v:daily}; if(idx>=0) rows[idx]=point; else rows.push(point); rows=rows.slice(-56); storage.setJSON(`inj_reward_heat_v4_${state.address||'guest'}`,rows);
  const max=Math.max(...rows.map(x=>number(x.v)),daily,1e-9); const padded=[...Array(Math.max(0,56-rows.length)).fill(null),...rows];
  host.innerHTML=padded.map(row=>{ if(!row) return '<i class="heat-cell"></i>'; const ratio=row.v/max; const level=Math.max(1,Math.min(4,Math.ceil(ratio*4))); return `<i class="heat-cell l${level}" title="${escapeHtml(row.d)} · ${inj(row.v,6)} INJ"></i>`; }).join('');
}
...
function renderInsights(total,nw,daily){
  ...
  renderHeatmap(); renderAchievements(total,daily);
}
...
function clearWalletState(){
  state.available=0; state.staked=0; state.rewards=0; state.apr=0; state.networkApr=0; state.communityTax=0; state.weightedCommission=0; state.validators=[]; state.rewardHistory=[]; state.rewardHistoryLoaded=false; state.rewardHistoryLoading=false; state.rewardHistoryNextKey=''; state.lastAccountUpdate=0;
}
...
(() => {
  ...
  function decodeEventValue(value){
    ...
    if(/^[A-Za-z0-9+/]+={0,2}$/.test(text) && text.length%4===0){
      try{ const decoded=atob(text); if(/^[\x20-\x7E\r\n\t]+$/.test(decoded)) return decoded; }catch{}
    }
    return text;
  }
  function eventAttrs(event){
    const out={};
    for(const attr of event?.attributes||[]){
      const key=decodeEventValue(attr?.key); const value=decodeEventValue(attr?.value);
      if(key){ if(out[key]===undefined) out[key]=value; else out[key]=`${out[key]},${value}`; }
    }
    return out;
  }
  function flattenMessages(messages=[]){
    ...
    for(const message of messages||[]){
      ...
      result.push(message);
      const nested=message.msgs||message.messages||message.value?.msgs||message.value?.messages;
      if(Array.isArray(nested)) result.push(...flattenMessages(nested));
    }
    return result;
  }
  function messageType(message){ return String(message?.['@type']||message?.type_url||message?.type||''); }
  function messageOwner(message){ return String(message?.delegator_address||message?.delegatorAddress||message?.sender||message?.grantee||''); }
  function messageAmountInj(message){
    const amount=message?.amount?.amount ?? message?.value?.amount?.amount ?? message?.amount;
    const denom=message?.amount?.denom ?? message?.value?.amount?.denom ?? 'inj';
    if(String(denom)!=='inj') return 0;
    return fromWei(amount);
  }
  function txHashOf(response,tx){ return String(response?.txhash||response?.tx_hash||tx?.txhash||tx?.hash||''); }
  function txTimeOf(response,tx){ const raw=response?.timestamp||tx?.timestamp||tx?.blockTimestamp||Date.now(); const t=new Date(raw).getTime(); return Number.isFinite(t)?t:Date.now(); }
  function successfulTx(response){ return Number(response?.code||0)===0; }
  function withdrawalAmountFromEvents(response,address){
    const events=response?.events||response?.logs?.flatMap(log=>log?.events||[])||[];
    let amount=0;
    for(const event of events){
      const attrs=eventAttrs(event);
      if(event?.type==='withdraw_rewards') amount+=injFromCoinString(attrs.amount);
    }
    if(amount>0) return amount;
    for(const event of events){
      if(event?.type!=='coin_received') continue;
      const attrs=eventAttrs(event);
      if(!attrs.receiver || attrs.receiver===address) amount+=injFromCoinString(attrs.amount);
    }
    return amount;
  }
  function activitySeenKey(address){ return `inj_v5_activity_seen_${walletKey(address)}`; }
  async function fetchActivityTransactions(address){
    const expressions=[
    ...
    for(const expression of expressions){
      try{
        const path=`/cosmos/tx/v1beta1/txs?events=${encodeURIComponent(expression)}&pagination.limit=100&order_by=ORDER_BY_DESC`;
        const data=await lcd(path);
        const txs=data?.txs||[]; const responses=data?.tx_responses||[];
        for(let i=0;i<Math.max(txs.length,responses.length);i++){
          const tx=txs[i]||{}; const response=responses[i]||{}; const hash=txHashOf(response,tx);
          if(hash&&!merged.has(hash)) merged.set(hash,{tx,response});
        }
      }catch(error){ console.warn('Activity query non disponibile',expression,error); }
    }
    return [...merged.values()];
  }
  async function syncOnChainActivity(){
    const address=state.address; if(!validAddress(address)) return;
    const rows=await fetchActivityTransactions(address); if(address!==state.address) return;
    const seen=new Set(storage.getJSON(activitySeenKey(address),[]));
    const discovered=[];
    for(const {tx,response} of rows){
      if(!successfulTx(response)) continue;
      const hash=txHashOf(response,tx); if(!hash) continue;
      const messages=flattenMessages(tx?.body?.messages||tx?.tx?.body?.messages||[]);
      const withdrawals=messages.filter(m=>messageType(m).endsWith('MsgWithdrawDelegatorReward') && (!messageOwner(m)||messageOwner(m)===address));
      const delegates=messages.filter(m=>messageType(m).endsWith('MsgDelegate') && (!messageOwner(m)||messageOwner(m)===address));
      const redelegates=messages.filter(m=>messageType(m).endsWith('MsgBeginRedelegate') && (!messageOwner(m)||messageOwner(m)===address));
      const when=txTimeOf(response,tx);
      if(withdrawals.length){
        const id=`${hash}:withdraw`; if(!seen.has(id)){
          const amount=withdrawalAmountFromEvents(response,address);
          discovered.push({id,t:when,kind:'reward',text:`Reward prelevati: ${amount>0?`${inj(amount,6)} INJ`:'quantità registrata on-chain'} · ${hash.slice(0,8)}…`});
        }
      }
      delegates.forEach((message,index)=>{
        const id=`${hash}:delegate:${index}`; if(seen.has(id)) return;
        const amount=messageAmountInj(message);
        discovered.push({id,t:when,kind:'stake',text:`INJ rimessi in staking: ${amount>0?`${inj(amount,6)} INJ`:'quantità non disponibile'} · ${hash.slice(0,8)}…`});
      });
      redelegates.forEach((message,index)=>{
        const id=`${hash}:redelegate:${index}`; if(seen.has(id)) return;
        const amount=messageAmountInj(message);
        discovered.push({id,t:when,kind:'stake',text:`Redelega effettuata: ${amount>0?`${inj(amount,6)} INJ`:'quantità non disponibile'} · ${hash.slice(0,8)}…`});
      });
    }
    discovered.sort((a,b)=>a.t-b.t).forEach(item=>{
      seen.add(item.id);
      V5.activities.unshift({t:item.t,text:item.text,kind:item.kind});
    });
    if(discovered.length){
      V5.activities=V5.activities.sort((a,b)=>b.t-a.t).slice(0,100);
      storage.setJSON('inj_v5_activity',V5.activities);
      storage.setJSON(activitySeenKey(address),[...seen].slice(-1000));
      renderActivity();
    }
  }
  ...
  loadAccount=async function(...args){const start=performance.now();const result=await originalLoadAccount.apply(this,args);V5.apiLatency=Math.round(performance.now()-start);if(state.address){await loadSnapshots();saveSnapshot();await syncOnChainActivity()}renderHealth();renderSmartInsights();renderMonitor();return result};
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    await openDb(); if(state.address){await loadSnapshots();await syncOnChainActivity();} renderActivity();renderEvents();renderHealth();renderSmartInsights();renderMonitor();updateTimeline();
    document.addEventListener('inj:wallet-changed',async()=>{ V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); });
    q('clearActivity')?.addEventListener('click',()=>{V5.activities=[];storage.setJSON('inj_v5_activity',[]);renderActivity()});
    q('focusButton')?.addEventListener('click',()=>document.body.classList.toggle('focus-mode'));
    document.addEventListener('keydown',e=>{if((e.key==='f'||e.key==='F')&&!/input|textarea/i.test(e.target.tagName))document.body.classList.toggle('focus-mode')});
    ...
    q('addEventButton')?.addEventListener('click',()=>q('eventDialog').showModal());q('saveEvent')?.addEventListener('click',e=>{e.preventDefault();const title=q('eventTitle').value.trim(),t=new Date(q('eventDate').value).getTime();if(!title||!t){toast('Compila titolo e data');return}V5.events.push({title,t});storage.setJSON('inj_v5_events',V5.events);renderEvents();q('eventDialog').close();q('eventForm').reset();addActivity(`Evento aggiunto: ${title}`)});q('eventList')?.addEventListener('click',e=>{const b=e.target.closest('[data-i]');if(!b)return;V5.events.splice(number(b.dataset.i),1);storage.setJSON('inj_v5_events',V5.events);renderEvents()});
    setInterval(()=>{renderEvents();renderMonitor();renderHealth()},10000);
  });
})();
