...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    document.addEventListener('inj:wallet-changed',async()=>{ state.rewardHistory=savedRewardHistory(state.address); state.rewardHistoryLoaded=state.rewardHistory.length>0; state.rewardHistoryNextKey=''; renderRewardHistory(); V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); startRealtimeWallet(state.address); });
