...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    await openDb(); if(state.address)await loadSnapshots(); renderActivity();renderEvents();renderHealth();renderSmartInsights();renderMonitor();updateTimeline(); if(state.address)startRealtimeWallet(state.address);
    document.addEventListener('inj:wallet-changed',async()=>{ V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); startRealtimeWallet(state.address); });
