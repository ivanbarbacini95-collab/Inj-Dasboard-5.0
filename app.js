...
function switchView(name){
  ...
  ['dashboard','portfolio','market','withdrawals'].forEach(v=>{ const el=$(v+'View'); if(el) el.classList.toggle('active',v===name); });
...
function render(){
  ...
  setText('targetCurrent',`${inj(state.staked,2)} INJ`); setText('targetMax',`${inj(state.target,0)} INJ`);
  const pct=Math.min(100,state.target?state.staked/state.target*100:0); $('targetFill').style.width=`${pct}%`; setText('targetPercent',`${pct.toFixed(2)}% completato`);
}
...
function drawAll(){
  ...
  drawChart($('portfolioNetWorthChart'),state.netWorthHistory,true,state.hover.portfolioNetWorthChart);
...
function bindInteractiveChart(canvasId,tooltipId,getRows,type){
  ...
  canvas.addEventListener('pointerdown',e=>{ pointerId=e.pointerId; startX=lastX=e.clientX; dragging=false; canvas.setPointerCapture?.(pointerId); canvas.classList.add('grabbing'); });
  canvas.addEventListener('pointermove',e=>{
    if(pointerId!==null&&e.pointerId===pointerId){
      const dx=e.clientX-lastX; if(Math.abs(e.clientX-startX)>5) dragging=true;
      if(dragging){ const total=chartRows(getRows()).length; const view=state.chartViews[canvasId]; const pxPerPoint=canvas.getBoundingClientRect().width/Math.max(1,(view?.count||total)); const steps=Math.round(-dx/Math.max(2,pxPerPoint)); if(steps) panChart(canvasId,total,steps); lastX=e.clientX; tip.classList.remove('show'); }
      else showHover(e.clientX,e.clientY);
    } else showHover(e.clientX,e.clientY);
  });
  const finish=e=>{ if(pointerId!==null&&(!e||e.pointerId===pointerId)){ try{canvas.releasePointerCapture?.(pointerId)}catch{} pointerId=null; canvas.classList.remove('grabbing'); if(!dragging&&e) showHover(e.clientX,e.clientY); dragging=false; } };
  canvas.addEventListener('pointerup',finish); canvas.addEventListener('pointercancel',finish);
  canvas.addEventListener('pointerleave',e=>{ if(pointerId===null){delete state.hover[canvasId];tip.classList.remove('show');drawAll()} });
...
function initEvents(){
  ...
  bindInteractiveChart('portfolioNetWorthChart','portfolioNetWorthTooltip',()=>state.netWorthHistory,'networth');
  ...
  $('targetButton').addEventListener('click',()=>{ $('targetInput').value=state.target; $('targetDialog').showModal(); });
  $('saveTarget').addEventListener('click',()=>{ const v=Math.max(1,number($('targetInput').value)); state.target=v; storage.set('inj_target',v); render(); });
...
function init(){
  ...
  state.target=Math.max(1,number(storage.get('inj_target','1000'))||1000); const saved=storage.get('inj_address',''); $('addressInput').value=saved;
