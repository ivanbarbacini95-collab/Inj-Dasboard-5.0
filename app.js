...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    status('online','Online'); render(); drawAll(); if(!state.rewardHistoryLoaded&&!state.rewardHistoryLoading) loadRewardHistory({showFeedback:false}); if(showFeedback) toast('Wallet aggiornato');
...
function renderRewardWithdrawalChart(){
  ...
  barsHost.innerHTML=rows.map((row,index)=>{
    const height=Math.max(4,(number(row.amount)/max)*100);
    ...
    return `<div class="reward-bar-column" title="${escapeHtml(full)} · ${inj(row.amount,6)} INJ"><span class="reward-bar-value">${inj(row.amount,4)}</span><i class="reward-bar" style="height:${height}%"></i><small>${escapeHtml(label)}</small></div>`;
  }).join('');
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    await openDb(); if(state.address)await loadSnapshots(); renderActivity();renderHealth();renderSmartInsights();renderMonitor();updateTimeline(); if(state.address)startRealtimeWallet(state.address);
    document.addEventListener('inj:wallet-changed',async()=>{ state.rewardHistory=[]; state.rewardHistoryLoaded=false; state.rewardHistoryNextKey=''; V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); startRealtimeWallet(state.address); });
