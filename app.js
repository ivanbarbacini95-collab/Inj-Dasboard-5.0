...
async function loadAccount(showFeedback=true){
  ...
  try{
    ...
    const refreshScrollX=window.scrollX;
    const refreshScrollY=window.scrollY;
    ...
    status('online','Online'); render(); drawAll();
    requestAnimationFrame(()=>window.scrollTo(refreshScrollX,refreshScrollY));
    if(showFeedback) toast('Wallet aggiornato');
...
function renderWalletTabs(){
  ...
  if(!state.wallets.length){
    host.innerHTML='<span class="wallet-tab-empty">Aggiungi un wallet con ＋</span>';
...
(() => {
  const V5 = {
    ...
    accountState: storage.getJSON('inj_v5_account_state', {}) || {},
    seenTx: storage.getJSON('inj_v5_seen_activity_txs', {}) || {},
  ...
  function renderActivity(){
    ...
    box.innerHTML=V5.activities.length?V5.activities.map(a=>`<div class="activity-row"><time>${new Intl.DateTimeFormat('it-IT',{hour:'2-digit',minute:'2-digit'}).format(new Date(a.t))}</time><strong>${escapeHtml(a.text)}</strong></div>`).join(''):'<p class="empty-line">In attesa di attività…</p>';
  }

  function detectAccountActivity(address, current){
    const key=walletKey(address);
    if(!key) return;
    const previous=V5.accountState[key];
    const next={
      available:number(current.available),
      staked:number(current.staked),
      rewards:number(current.rewards),
      updated:Date.now()
    };
    V5.accountState[key]=next;
    storage.setJSON('inj_v5_account_state',V5.accountState);
    if(!previous) return;

    const rewardDrop=number(previous.rewards)-next.rewards;
    const stakedIncrease=next.staked-number(previous.staked);
    const availableIncrease=next.available-number(previous.available);
    const epsilon=0.000001;

    if(rewardDrop>epsilon){
      addActivity(`Ritiro reward rilevato: ${inj(rewardDrop,6)} INJ`,'reward');
    }
    if(stakedIncrease>epsilon){
      const label=rewardDrop>epsilon?'Reward rimessi in staking':'Nuova delega / re-stake';
      addActivity(`${label}: ${inj(stakedIncrease,6)} INJ`,'stake');
    } else if(rewardDrop>epsilon && availableIncrease>epsilon){
      addActivity(`Reward accreditati nel saldo disponibile: ${inj(Math.min(rewardDrop,availableIncrease),6)} INJ`,'reward');
    }
  }

  function messageType(message){ return String(message?.['@type']||message?.type_url||''); }
  function coinAmountInj(coin){ return coin?.denom==='inj'?fromWei(coin.amount):0; }

  async function scanOnChainActivity(address){
    if(!validAddress(address)||V5.activityScanRunning) return;
    V5.activityScanRunning=true;
    try{
      const senderEvent=encodeURIComponent(`message.sender='${address}'`);
      const data=await lcd(`/cosmos/tx/v1beta1/txs?events=${senderEvent}&pagination.limit=50&order_by=ORDER_BY_DESC`);
      const txs=data?.txs||[];
      const responses=data?.tx_responses||[];
      const key=walletKey(address);
      const seen=new Set(Array.isArray(V5.seenTx[key])?V5.seenTx[key]:[]);
      const discovered=[];

      for(let i=0;i<txs.length;i++){
        const tx=txs[i], response=responses[i]||{};
        if(Number(response?.code||0)!==0) continue;
        const hash=String(response?.txhash||'');
        if(!hash||seen.has(hash)) continue;
        const timestamp=response?.timestamp?new Date(response.timestamp).getTime():0;
        const messages=txMessages(tx);
        const withdrawals=parseWithdrawalTx(tx,response,address);

        for(const row of withdrawals){
          discovered.push({t:timestamp||Date.now(),hash,text:`Ritiro reward on-chain: ${inj(row.amount,6)} INJ`,kind:'reward'});
        }

        for(const message of messages){
          const type=messageType(message);
          if(type.endsWith('MsgDelegate')&&message?.delegator_address===address){
            const amount=coinAmountInj(message?.amount);
            if(amount>0) discovered.push({t:timestamp||Date.now(),hash,text:`INJ rimessi in staking: ${inj(amount,6)} INJ`,kind:'stake'});
          } else if(type.endsWith('MsgBeginRedelegate')&&message?.delegator_address===address){
            const amount=coinAmountInj(message?.amount);
            if(amount>0) discovered.push({t:timestamp||Date.now(),hash,text:`Redelega / re-stake: ${inj(amount,6)} INJ`,kind:'stake'});
          }
        }
        seen.add(hash);
      }

      discovered.sort((a,b)=>a.t-b.t).forEach(item=>addActivity(item.text,item.kind));
      V5.seenTx[key]=[...seen].slice(-300);
      storage.setJSON('inj_v5_seen_activity_txs',V5.seenTx);
    }catch(error){
      console.warn('Activity on-chain scan',error);
    }finally{
      V5.activityScanRunning=false;
    }
  }
  ...
  loadAccount=async function(...args){
    ...
    const requestedAddress=state.address;
    const previousUpdate=state.lastAccountUpdate;
    const result=await originalLoadAccount.apply(this,args);
    V5.apiLatency=Math.round(performance.now()-start);
    const syncSucceeded=requestedAddress&&requestedAddress===state.address&&state.lastAccountUpdate>previousUpdate;
    if(syncSucceeded){
      detectAccountActivity(requestedAddress,{available:state.available,staked:state.staked,rewards:state.rewards});
      await scanOnChainActivity(requestedAddress);
      await loadSnapshots();
      saveSnapshot();
      addActivity('Wallet sincronizzato');
    }
    renderHealth();renderSmartInsights();renderMonitor();return result;
  };
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    await openDb(); if(state.address)await loadSnapshots(); renderActivity();renderEvents();renderHealth();renderSmartInsights();renderMonitor();updateTimeline();
