...
(() => {
  ...
  function growthRows(){
    return safeJson(growthKey(),[])
      .filter(x=>Number.isFinite(Number(x?.staked))&&Number(x?.t)>0&&(x.type==='baseline'||x.type==='compound'))
      .sort((a,b)=>a.t-b.t);
  }
  ...
  function drawGrowth(){
    if(drawQueued)return; drawQueued=true; requestAnimationFrame(()=>{drawQueued=false;
      ...
      data.forEach((p,i)=>{ctx.beginPath();ctx.arc(x(i),y(p.staked),p.type==='compound'?3.4:2.5,0,Math.PI*2);ctx.fillStyle=p.type==='compound'?'#fbbf24':'#22d3a6';ctx.fill()});
    })}
