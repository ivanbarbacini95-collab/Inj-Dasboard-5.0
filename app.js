...
function loadWallet(){ const value=$('addressInput').value.trim(); if(!validAddress(value)){ toast('Indirizzo non valido'); return; } closeWalletSearch(); state.address=value; state.netWorthHistory=normalizeHistory(storage.getJSON(netWorthHistoryKey(value),[])).slice(-HISTORY.netWorthLimit); state.rewardHistory=[]; state.rewardHistoryLoaded=false; state.rewardHistoryNextKey=''; clearInterval(state.accountTimer); loadAccount(); state.accountTimer=setInterval(()=>loadAccount(false),30000); }
function initEvents(){
...
function init(){
  ...
  initEvents(); setInterval(updateSyncCountdown,1000); updateSyncCountdown(); render(); renderMarket(); loadMarket(); loadMarketTimeframe(state.timeframe); connectPriceSocket();
