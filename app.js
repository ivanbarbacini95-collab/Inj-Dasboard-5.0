...
function addRewardWithdrawal(row,address=state.address){
  ...
  const rows=[...unique.values()].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,250);
...
function renderPriceSimulator(){
  ...
  if(delta){
    if(!price) delta.textContent='Inserisci valori validi';
      ...
      const change=(price/state.price-1)*100;
  ...
  setText('simulatedWalletUsd',state.address&&price>0?money(walletTotal*price):'—');
  ...
  if(walletStatus){
    ...
    else walletStatus.textContent=`Controvalore teorico a ${preciseMoney(price,price<1?4:2)} per INJ`;
  }
...
function renderNetworkTokenomics(){
  ...
  setText('networkStakingApr',`${apr.toLocaleString('it-IT',{minimumFractionDigits:2,maximumFractionDigits:2})}%`);
...
function initPriceSimulator(){
  ...
  if(cap) cap.value=storage.get('inj_sim_market_cap','1B');
  cap?.addEventListener('input',renderPriceSimulator);
  document.querySelectorAll('[data-sim-cap]').forEach(button=>button.addEventListener('click',()=>{if(cap){cap.value=button.dataset.simCap;renderPriceSimulator();}}));
  const cached=number(storage.get('inj_live_total_supply','0'));
...
function startStartupLoader(){
  ...
  const timer=setInterval(()=>{
    ...
    if(elapsed>=maxDuration){stop();showStartupError('Impossibile collegarsi ai servizi Injective');}
  },150);
...
function init(){
  ...
  initEvents(); initPriceSimulator(); renderWalletTabs(); setInterval(updateSyncCountdown,1000); updateSyncCountdown(); render(); renderMarket(); loadMarket(); loadMarketTimeframe(state.timeframe); connectPriceSocket();
