...
(()=>{
  ...
  byId('drawerDataMode')?.addEventListener('click',()=>byId('dataModeButton')?.click());
  byId('drawerPrivacy')?.addEventListener('click',()=>byId('privacyButton')?.click());
  byId('drawerTheme')?.addEventListener('click',()=>byId('themeButton')?.click());
  const motion=byId('motionToggle'), drawerMotion=byId('drawerMotionToggle');
  if(motion&&drawerMotion){drawerMotion.checked=motion.checked;drawerMotion.addEventListener('change',()=>{motion.checked=drawerMotion.checked;motion.dispatchEvent(new Event('change',{bubbles:true}))});motion.addEventListener('change',()=>drawerMotion.checked=motion.checked)}
})();

/* v7.1.2 visibility self-check */
document.addEventListener('DOMContentLoaded',()=>{
  ['menuControl','menuToggle','headerSearch','searchToggle','multiwalletVisualize','walletTabs','addWalletTab'].forEach(id=>{const el=document.getElementById(id);if(el){el.hidden=false;el.style.removeProperty('display');el.style.visibility='visible';el.style.opacity='1';}});
});
